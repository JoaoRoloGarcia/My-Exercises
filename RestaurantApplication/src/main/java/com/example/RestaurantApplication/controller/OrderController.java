...
public class OrderController {
	...
	public OrderResponse addOrder(@RequestBody OrderDto orderDto) {
		...
		try {
			...
			orderModel.setOrder_status(FoodStatus.ACCEPTED);
			...
			retValues.setMsg("You are doing something wrong");
			retValues.setSentOn(LocalDateTime.now(ZoneId.of("GMT")));
			retValues.setResValues(orderDto);
			e.getMessage();
		}
	...
	// NÃO FEITO, PERCEBER LÓGICA DE MEXER EM ENUMS
	@GetMapping("/foodStatus")
	public OrderModel changeFoodStatus(@RequestBody OrderDto orderDto) {
		OrderResponse retValues = new OrderResponse();
		FoodStatus foodStatus = orderService.getOrderStatus(orderDto.getDish_id());
		Converter converter = new Converter();
		OrderModel orderModel = orderService.save(converter.convert(orderDto));

		switch (foodStatus) {
		case ACCEPTED:
			retValues.setFoodStatus(FoodStatus.PREPARING);
			retValues.setSentOn(LocalDateTime.now(ZoneId.of("GMT")));
			break;
		case PREPARING:
			retValues.setFoodStatus(FoodStatus.DELIVERING);
			retValues.setSentOn(LocalDateTime.now(ZoneId.of("GMT")));
			break;
		case DELIVERING:
			retValues.setFoodStatus(FoodStatus.DELIVERED);
			retValues.setSentOn(LocalDateTime.now(ZoneId.of("GMT")));
		}
